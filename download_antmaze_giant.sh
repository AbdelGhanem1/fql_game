...
# FIXED: Using 'navigate' instead of 'play'
...
TARGET_DIR="$HOME/abdelghani_work/datasets/antmaze-giant"

mkdir -p "$TARGET_DIR"

# FIXED: Looking for 'navigate' shards
if ls "$OGBENCH_CACHE"/antmaze-giant-navigate*.npz 1> /dev/null 2>&1; then
    mv "$OGBENCH_CACHE"/antmaze-giant-navigate*.npz "$TARGET_DIR/"
    FILE_COUNT=$(ls -1 "$TARGET_DIR"/*.npz | wc -l)
    echo "🎉 Success! $FILE_COUNT dataset shards are now sitting in $TARGET_DIR."
else
    echo "⚠️  No antmaze-giant shards found in the cache. Download may have failed."
fi
