...
TARGET_DIR="$HOME/abdelghani_work/datasets/humanoidmaze-large"

mkdir -p "$TARGET_DIR"

# FIXED: Looking for 'humanoidmaze-large-navigate' shards (captures both train & val)
if ls "$OGBENCH_CACHE"/humanoidmaze-large-navigate*.npz 1> /dev/null 2>&1; then
    mv "$OGBENCH_CACHE"/humanoidmaze-large-navigate*.npz "$TARGET_DIR/"
    FILE_COUNT=$(ls -1 "$TARGET_DIR"/*.npz | wc -l)
    echo "🎉 Success! $FILE_COUNT dataset shards (train & val) are now sitting in $TARGET_DIR."
else
    echo "⚠️  No humanoidmaze-large shards found in the cache. Download may have failed."
fi
