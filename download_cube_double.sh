...
TARGET_DIR="$HOME/abdelghani_work/datasets/cube-double"

mkdir -p "$TARGET_DIR"

if ls "$OGBENCH_CACHE"/cube-double-play*.npz 1> /dev/null 2>&1; then
    mv "$OGBENCH_CACHE"/cube-double-play*.npz "$TARGET_DIR/"
    FILE_COUNT=$(ls -1 "$TARGET_DIR"/*.npz | wc -l)
    echo "🎉 Success! $FILE_COUNT dataset shards are now sitting in $TARGET_DIR."
else
    echo "⚠️  No cube-double shards found in the cache. Download may have failed."
fi
