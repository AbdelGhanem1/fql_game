...
# Usage: ./run_docker_antmaze.sh [JOB_INDEX]
JOB_INDEX=${1:-0}
...
if [ ! -d "$DATASET_DIR" ]; then
    echo "Loading antmaze-giant dataset into RAM disk..."
...
export WANDB_PROJECT="antmaze-giant_mirror_descent_v6"
...
# CRITICAL: We keep the Workstation_Repro run_group so the code natively finds the subdirectories of the models you just uploaded.
...
taskset -c 0-31 "$PYTHON_EXEC" main.py \
    ...
    --save_dir="/models/saved_models/job_${JOB_INDEX}_antmaze_giant_task${TASK_ID}_tauC${TAU_CRITIC}_tauS${TAU_SCORE}_mix${MIXTURE_PROB}"
